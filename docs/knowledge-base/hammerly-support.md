...
Open an active auction and submit a bid amount. A bid must be higher than the current bid. The auction must still be active, and a seller cannot bid on their own auction. A successful bid updates the current bid and appears in bid history. Signed-in users can view their bids and whether their latest bid is currently winning.
